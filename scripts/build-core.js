...
const buildModule = async (cssContent, format) => {
  ...
  fs.writeFileSync(`./${outputDir}/${format}/codersrank-summary.js`, fileContent);
};

const buildUMD = async (cssContent) => {
  const env = process.env.NODE_ENV || 'development';
  const filename = 'codersrank-summary';
  const outputDir = env === 'development' ? 'dev' : 'package';
  ...
  const bundle = await rollup({
    input: './src/codersrank-summary.js',
    plugins: [
      replace({
        delimiters: ['', ''],
        $_STYLES_$: cssContent,
...
const build = async () => {
  let cssContent = fs.readFileSync('./src/codersrank-summary.scss', 'utf-8');
